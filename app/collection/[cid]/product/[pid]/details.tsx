...
export default function DetailsPanel({ title, price, discount, options }: DetailsPanelProps) {
  return (
    <>
            ...
            {options.map((option) => (
              <div key={option.name} className='flex flex-col gap-5 mt-3'>
                ...
                <div className='flex flex gap-2'>
                  {option.values.map((value) => <OptionBox key={value} active={false} onClick={() => {return}}>{value}</OptionBox>)}
                </div>
              </div>
            ))}
            </HR>
