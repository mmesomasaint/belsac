...
export default function CartItems() {
  ...
  const extractAttributes = (lines: CartLine[]) => {
    return lines.map((line) => {
      const attributes = Object.fromEntries(line.attributes.map(({key, value}: Attribute) => [key, value]))
...
function CartItem({ line: {id, title, featuredImage, price, quantity} }: { line: CartLine }) {
  ...
  return (
    <div className='relative grid grid-cols-6 place-items-stretch min-h-[20rem] gap-10 p-10 border-b border-black/40 last:border-none'>
      ...
      {hasImage && <Image src={featuredImage} alt={title} className='w-full object-cover'/>}
      </div>
      ...
      <button type='button' onClick={() => console.log(`Line: ${id}, ready for editing!!`)} className='w-14 h-14 flex justify-center items-center border border-black hover:bg-black/10 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-black' title='Edit line'>
        <PiPencilSimpleLineThin className='text-2xl text-black' />
      </button>
      <button type='button' onClick={() => console.log(`Line: ${id}, deleted!!`)} className='w-14 h-14 flex justify-center items-center border border-red-500 hover:bg-red-100 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-black' title='Delete line'>
